...
<script>
	...
	function updateSize() {
		...
		height = Math.min(screenHeight * 0.8, 3000);
	...
	console.log(
		'HII',
		sankeyData.links.map((d) => d.value)
	);
</script>

<div class="relative w-full max-w-none rounded-xl p-6" bind:clientWidth={width}>
	<h1 class="text-xl font-bold text-gray-200 mb-6 text-center">Ideenreise</h1>
          ...
          <g transform="rotate(-90) translate(-{height},0)">-->
			...
			{#each sankeyDataReady.nodes as node}
				<g class="node" transform="translate({node.x0},{node.y0})">
					...
					{#if node.stage === 'Idee'}
						<rect class="fill-gray-200" x="0" y="0" width={10} height={10}> </rect>
					{/if}
				</g>
			{/each}
		</svg>
